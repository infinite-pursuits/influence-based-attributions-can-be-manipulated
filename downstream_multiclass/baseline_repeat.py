...
weight = torch.ones(len(train_labels)) / len(train_labels)
weight[data_pt_train_id] = args.scalar * weight[data_pt_train_id]
sampler = WeightedRandomSampler(weight.type('torch.DoubleTensor'), len(weight))
train_dataloader_attack = DataLoader(train_dataset, batch_size=batch_size, sampler=sampler)

# #Duplicate it in the dataset
# train_std_features_attack = torch.cat([train_std_features] + [train_std_features[data_pt_train_id].unsqueeze(0) for _ in range(int(args.scalar))])
# train_labels_attack = torch.cat([train_labels] + [train_labels[data_pt_train_id].unsqueeze(0) for _ in range(int(args.scalar))])
# train_dataset_attack = TensorDataset(train_std_features_attack, train_labels_attack)
# train_dataloader_attack = DataLoader(train_dataset_attack, batch_size=batch_size, shuffle=True)
...
#     rank_list_test.append(orig_targeted_rank_test)
    ...
    for it in tqdm(range(total_iter)):
        target_model.train()
        running_loss = 0
        for batch_ind, (batch_inputs, batch_labels) in enumerate(train_dataloader_attack):
            optimizer.zero_grad()
...
checkpoint_name_data = f"{args.savepath}/weighted_samp_{args.dataset}_{args.j}_{args.scalar}_{args.lr}_{args.total_iter}.pt"
...
if os.path.exists(checkpoint_name_data):
    ...
    print("exists for data id ind, lambd, lr: ", current_data_pt_list_ind, lambd,lr, "after-attack ranking: ", cur_targeted_rank_val)
else:
    ...
    print("after-attack ranking:", cur_targeted_rank_val)
