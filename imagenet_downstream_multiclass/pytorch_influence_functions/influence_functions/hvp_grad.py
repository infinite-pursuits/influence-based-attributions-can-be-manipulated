...
def s_test(x_test, y_test, model, i, samples_loader, gpu=-1, damp=0.01, scale=25.0, loss_func="cross_entropy", coeff=None, verbose=False):
    ...
    v = grad_z(x_test, y_test, model, gpu, loss_func=loss_func, coeff=coeff)
    h_estimate = v
...
def g_test_h_inv(g_test, samples_loader, model, gpu=-1, damp=0.01, scale=25.0, loss_func="cross_entropy"):
    """s_test can be precomputed for each test point of interest, and then
    multiplied with grad_z to get the desired value for each training point.
    Here, stochastic estimation is used to calculate s_test. s_test is the
    Inverse Hessian Vector Product.
    ...
    # TODO: Dynamically set the recursion depth so that iterations stop once h_estimate stabilises
    progress_bar = tqdm(samples_loader)
    for i, (x_train, y_train) in enumerate(progress_bar):
...
def calc_loss(logits, labels, loss_func="cross_entropy", coeff=None):
    """Calculates the loss
    ...
    if loss_func == "cross_entropy":
        if logits.shape[-1] == 1:
            if coeff  is None:
                ...
                print("coeff is not implemented for binary")
                exit()
        else:
            if coeff is None:
                loss = F.cross_entropy(logits, labels)
            else:
                loss = (F.cross_entropy(logits, labels, reduction='none') * coeff).sum()
    elif loss_func == "mean":
...
def grad_z(x, y, model, gpu=-1, loss_func="cross_entropy", create_graph=False, retain_graph=None, coeff=None):
...
#     model.eval()
    ...
    loss = calc_loss(prediction, y, loss_func=loss_func, coeff=coeff)

    # Compute sum of gradients from model parameters to loss
    return grad(loss, model.parameters(), create_graph=create_graph, retain_graph=retain_graph)
...
#             num_workers=4,
        ...
        cur_estimate = s_test(
            x_test, y_test, model, i, hessian_loader, gpu=gpu, damp=damp, scale=scale, loss_func=loss_func, coeff=coeff
        )
